object Exercise_16_0_1 {
  def main(args: Array[String]) {
    // \x.((cons x) nil)
    // val result = testInfer.showType(Lam("x", App(App(Var("cons"), Var("x")), Var("nil"))))
    // println(result)

    // by Nada
    val result = testInfer.showType(
      LetRec("length",
        Lam("xs", App(App(App(Var("if"), App(Var("isEmpty"), Var("xs"))),
          Var("zero")), App(Var("succ"), App(Var("length"), App(Var("tail"),
          Var("xs")))))), Var("length")))
    println(result)
  }
}
...
object typeInfer {
  ...
  def tp(env: Env, e: Term, t: Type, s: Subst): Subst = {
    ...
    e match {
      ...
      case LetRec(x, e1, e2) =>
        val a = newTyvar()
        val s1 = tp((x, gen(env, a)) :: env, e1, a, s)
        tp((x, gen(env, s1(a))) :: env, e2, t, s)
    }
  }
